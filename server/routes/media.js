...
const { persistImage, persistPdf, deleteStoredFile } = require('../services/mediaService');
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const filter = {};
    if (req.query.kind && ['image', 'pdf', 'other'].includes(req.query.kind)) filter.kind = req.query.kind;
    if (req.query.q) {
    ...
    res.json({ items: withUsage, pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
  })
);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const doc = await Media.findByIdAndUpdate(req.params.id, { $set: update }, { new: true });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    Object.assign(doc, meta, { alt: doc.alt, caption: doc.caption, featured: doc.featured });
    await doc.save();
    usageService.invalidate();
    res.json(doc);
  })
