...
const { signToken, setAuthCookie, clearAuthCookie, requireAuth, authorize } = require('../middleware/auth');
...
const publicUser = (u) => ({ _id: u._id, name: u.name, email: u.email, role: u.role, lastLoginAt: u.lastLoginAt });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const dummyHash = '$2a$10$C6UzMDM.H6dfI/f/IKcEeO7ZBDSfZ1NbeiJ/1mVCQdNVXA03vbW0W';
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ error: errors.array()[0].msg, code: 'VALIDATION' });
    const user = await User.findById(req.user._id);
    const ok = await bcrypt.compare(req.body.currentPassword, user.passwordHash);
    if (!ok) return res.status(400).json({ error: 'Current password is incorrect', code: 'BAD_CREDENTIALS' });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (exists) return res.status(409).json({ error: 'Email already in use', code: 'DUPLICATE' });
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.body.role && ['admin', 'editor'].includes(req.body.role)) update.role = req.body.role;
    if (req.body.name) update.name = String(req.body.name).slice(0, 120);
    if (req.params.id === req.user._id.toString() && update.active === false) {
      return res.status(400).json({ error: 'You cannot deactivate your own account', code: 'SELF_DEACTIVATE' });
    }
