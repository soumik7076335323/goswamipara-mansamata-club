...
import { EmptyState, CardSkeletons, ErrorState, Pagination, Img } from '../../components/ui';
import { videoEmbedUrl, videoThumb } from '../../utils/format';

export function VideoCard({ video }) {
  const { L, t } = useLanguage();
  const [playing, setPlaying] = useState(false);
  const thumb = video.thumbnail || videoThumb(video.provider, video.videoId);
  return (
    <article className="card">
      {playing ? (
        ...
        <button type="button" className="video-thumb" onClick={() => setPlaying(true)} aria-label={`${t('common.watchVideo')}: ${L(video.title)}`}>
          {thumb ? <Img src={thumb} alt="" style={{ width: '100%', aspectRatio: '16/9', objectFit: 'cover' }} /> : <div style={{ width: '100%', aspectRatio: '16/9', background: '#14100c' }} />}
          <span className="video-play" aria-hidden="true">▶</span>
        </button>
      )}
      <div className="card-body">
        {video.category && <span className="chip chip-gold">{video.category}</span>}
      ...
      <section className="section">
        <div className="container">
          ...
          {loading ? <CardSkeletons count={6} /> : error ? <ErrorState onRetry={refetch} /> : !data || data.items.length === 0 ? (
            <EmptyState icon="🎬" title={t('videos.empty')} text={t('common.empty.admin')} />
          ) : (
            <>
              <div className="grid grid-3">{data.items.map((v) => <VideoCard key={v._id} video={v} />)}</div>
