...
export default function CrudPage({ config }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(data.items);
      setPagination(
        data.pagination || { page: 1, pages: 1, total: data.items.length },
      );
    } catch (err) {
      ...
      case "pdf":
        return (
          <PdfField
            ...
            onChange={(url, name) =>
              setForm((o) => ({ ...o, [f.name]: url, [f.nameField]: name }))
            }
          />
        );
      case "videoFile":
    ...
    <div>
      <div className="admin-page-head">
        <div>
          ...
          <p>
            {t("admin.list.total")}: {pagination.total}
          </p>
        </div>
        {canAdd && (
          <button
        ...
        {filters.map((f) => (
          <select
            ...
            onChange={(e) => {
              setFilterVals((o) => ({
                ...o,
                [f.param]: e.target.value || undefined,
              }));
              setPage(1);
            }}
      ...
      <Modal
        ...
        title={editing ? t("common.edit") : t("admin.list.addNew")}
        wide
      >
        {formError && (
          <div
