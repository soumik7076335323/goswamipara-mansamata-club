...
export default function MediaLibrary() {
  ...
  const load = useCallback(async () => {
    try {
      const { data } = await api.get("/api/media", {
        params: { page, limit: 24, q: q || undefined, kind: kind || undefined },
      });
      setItems(data.items);
  ...
  const handleUpload = async (files, mode = "image") => {
    const list = Array.from(files || []);
    if (!list.length) return;
    if (mode === "image") {
      if (list.some((f) => !IMAGE_MIMES.includes(f.type))) {
        toast.error(t("admin.media.invalid"));
        return;
      }
      if (list.some((f) => f.size > MAX_IMAGE_MB * 1024 * 1024)) {
    ...
    if (mode === "video") {
      ...
      if (list[0].size > MAX_VIDEO_MB * 1024 * 1024) {
        toast.error(t("admin.media.tooLarge"));
        return;
      }
    }
    ...
    setProgress(0);
    try {
      if (mode === "pdf") await uploadPdf(list[0]);
      else if (mode === "video") await uploadVideo(list[0], setProgress);
  ...
  const doDelete = async (force = false) => {
    try {
      await api.delete(
        `/api/media/${toDelete._id}${force ? "?force=true" : ""}`,
      );
  ...
  const saveMeta = async () => {
    try {
      await api.patch(`/api/media/${editMeta._id}`, {
        alt: editMeta.alt,
    ...
    <div>
      <div className="admin-page-head">
        <div>
          ...
          <p>
            {t("admin.list.total")}: {pagination.total || 0}
          </p>
        </div>
        <div className="flex gap-2 wrap">
          <button
            ...
            onClick={() => {
              ...
              if (el) {
                el.accept = IMAGE_MIMES.join(",");
                el.multiple = true;
                el.dataset.mode = "image";
                el.click();
              }
            }}
          >
          ...
          <input
            ...
            data-mode="image"
            onChange={(e) =>
              handleUpload(e.target.files, e.target.dataset.mode || "image")
            }
          />
          <button
            type="button"
          ...
          <button
            ...
            onClick={() => {
              const el = fileRef.current;
              if (el) {
                el.accept = VIDEO_MIMES.join(",");
                el.multiple = false;
                el.dataset.mode = "video";
                el.click();
              }
            }}
          >
            {uploading &&
            fileRef.current &&
            fileRef.current.dataset.mode === "video"
            ...
            >
              ...
              {m.usageCount > 0 && (
                <span className="media-usage">
                  {m.usageCount} {t("admin.media.usedIn")}
                </span>
              )}
            </button>
          ))}
      ...
      {/* Detail modal */}
      <Modal
        open={Boolean(detail) && !inUse}
        onClose={() => setDetail(null)}
        title={detail ? detail.originalName || detail.filename : ""}
      >
        {detail && (
          <div>
            {detail.kind === "pdf" ? (
            ...
            <div className="kv mt-2">
              ...
              {detail.width > 0 && (
                <>
                  ...
                  <dd>
                    ...
                    {Math.round(detail.size / 1024)} KB
                  </dd>
                </>
              )}
              <dt>Usage</dt>
              <dd>
                {detail.usageCount > 0
                  ? `${detail.usageCount} ${t("admin.media.usedIn")}`
                  : t("admin.media.unused")}
      ...
      >
        {editMeta && (
          <div>
            <LocalizedField
              label={t("admin.media.altText")}
              value={editMeta.alt}
              onChange={(v) => setEditMeta((m) => ({ ...m, alt: v }))}
            />
            <LocalizedField
              label={t("admin.media.caption")}
              value={editMeta.caption}
              onChange={(v) => setEditMeta((m) => ({ ...m, caption: v }))}
            />
      ...
      <Modal
        ...
        title={t("admin.media.inUseTitle")}
      >
          ...
          <ul className="list-plain" style={{ fontSize: "0.86rem" }}>
            {inUse.usedIn.map((u) => (
              <li key={u.id} style={{ padding: "4px 0" }}>
                • <strong>{u.collection}</strong>: {u.title}
              </li>
            ))}
          </ul>
        )}
