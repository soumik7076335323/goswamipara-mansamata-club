...
const thumb = (url) => <Img src={url} alt="" className="thumb" style={{ width: 52, height: 40, objectFit: 'cover', borderRadius: 6 }} />;
const loc = (v, L) => L(v, '—');
...
export const crudConfigs = {
  events: {
    ...
    filters: [{ param: 'published', labelKey: 'common.status', options: publishedOpts }],
    columns: [
      { labelKey: 'common.image', render: (i) => thumb(i.coverImage) },
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.title, L)}</strong> },
      { labelKey: 'common.date', render: (i) => dateCell(i.date) },
      { labelKey: 'common.category', render: (i) => i.category || '—' },
    ],
    fields: [
      { name: 'title', type: 'localized', label: 'Title / শিরোনাম', required: true },
      { name: 'description', type: 'localizedText', label: 'Description / বিবরণ' },
      ...
      { name: 'displayOrder', type: 'number', label: 'Order / ক্রম', hint: 'Lower first / ছোট সংখ্যা আগে' },
      { name: 'featured', type: 'checkbox', label: 'Featured / বিশেষ' },
      { name: 'published', type: 'checkbox', label: 'Published / প্রকাশিত', default: true },
    ],
  ...
  notices: {
    ...
    columns: [
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.title, L)}</strong> },
      { labelKey: 'common.date', render: (i) => dateCell(i.date) },
      { labelKey: 'field.attachment', render: (i) => (i.attachmentUrl ? '📄 PDF' : '—') },
    ],
    fields: [
      { name: 'title', type: 'localized', label: 'Title / শিরোনাম', required: true },
      { name: 'content', type: 'localizedText', label: 'Content / বিষয়বস্তু' },
      { name: 'date', type: 'date', label: 'Date / তারিখ' },
      { name: 'attachmentUrl', type: 'pdf', nameField: 'attachmentName', label: 'Attachment (PDF) / সংযুক্তি' },
      { name: 'pinned', type: 'checkbox', label: 'Pin to top / উপরে পিন করুন' },
      { name: 'published', type: 'checkbox', label: 'Published / প্রকাশিত', default: true },
    ],
    defaultItem: { published: true, date: new Date().toISOString().slice(0, 10) },
  },
  ...
  videos: {
    ...
    toggles: ['published', 'featured'],
    columns: [
      { labelKey: 'common.image', render: (i) => thumb(i.thumbnail || videoThumb(i.provider, i.videoId)) },
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.title, L)}</strong> },
      { labelKey: 'field.provider', render: (i) => i.provider },
      { labelKey: 'common.category', render: (i) => i.category || '—' },
    ],
    fields: [
      { name: 'title', type: 'localized', label: 'Title / শিরোনাম', required: true },
      { name: 'description', type: 'localizedText', label: 'Description / বিবরণ' },
      { name: 'url', type: 'text', label: 'Video URL (YouTube / Vimeo)', required: true },
      { name: 'thumbnail', type: 'image', label: 'Thumbnail / থাম্বনেইল (ঐচ্ছিক)' },
      { name: 'category', type: 'text', label: 'Category / বিভাগ' },
    ...
    mapForEdit: (item) => ({
      ...item,
      url: item.url || (item.provider === 'youtube' ? `https://youtu.be/${item.videoId}` : `https://vimeo.com/${item.videoId}`),
    }),
    toPayload: (form) => {
      const parsed = parseVideoUrl(form.url);
      if (!parsed) throw Object.assign(new Error('Invalid video URL — use a YouTube or Vimeo link'), { friendlyMessage: 'Invalid video URL — use a YouTube or Vimeo link' });
      return {
        title: form.title,
  ...
  gallery: {
    ...
    toggles: ['published', 'featured'],
    columns: [
      { labelKey: 'common.image', render: (i) => thumb(i.coverImage || (i.images[0] && i.images[0].url)) },
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.title, L)}</strong> },
      { labelKey: 'common.images', render: (i) => (i.images || []).length },
    ],
    fields: [
      { name: 'title', type: 'localized', label: 'Album title / অ্যালবামের নাম', required: true },
      { name: 'description', type: 'localizedText', label: 'Album description / বিবরণ' },
      { name: 'coverImage', type: 'image', label: 'Cover image / কভার ছবি' },
    ...
    mapForEdit: (item) => ({
      ...item,
      photoUrls: (item.images || []).slice().sort((a, b) => a.displayOrder - b.displayOrder).map((p) => p.url),
    }),
    toPayload: (form) => ({
      title: form.title,
      description: form.description,
      coverImage: form.coverImage || form.photoUrls[0] || '',
      images: (form.photoUrls || []).map((url, i) => ({ url, caption: {}, displayOrder: i })),
      featured: form.featured,
      published: form.published,
      displayOrder: Number(form.displayOrder) || 0,
    }),
  ...
  committee: {
    ...
    filters: [{ param: 'active', labelKey: 'common.status', options: activeOpts }],
    columns: [
      { labelKey: 'common.image', render: (i) => thumb(i.photo) },
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.name, L)}</strong> },
    ...
    fields: [
      ...
      { name: 'displayOrder', type: 'number', label: 'Order / ক্রম' },
      { name: 'featured', type: 'checkbox', label: 'Featured / বিশেষ' },
      { name: 'active', type: 'checkbox', label: 'Active / সক্রিয়', default: true },
    ],
  ...
  members: {
    ...
    columns: [
      ...
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.name, L)}</strong> },
      { label: 'Designation / পদবি', render: (i, L) => loc(i.designation, L) },
      { labelKey: 'common.phone', render: (i) => (i.showPhone && i.phone ? i.phone : '🔒') },
      { labelKey: 'common.order', render: (i) => i.displayOrder },
    ],
    fields: [
      { name: 'name', type: 'localized', label: 'Name / নাম', required: true },
      ...
      { name: 'bio', type: 'localizedText', label: 'Bio / পরিচিতি' },
      { name: 'showPhone', type: 'checkbox', label: 'Show phone publicly / ফোন প্রকাশ' },
      { name: 'showAddress', type: 'checkbox', label: 'Show address publicly / ঠিকানা প্রকাশ' },
      { name: 'displayOrder', type: 'number', label: 'Order / ক্রম' },
      { name: 'featured', type: 'checkbox', label: 'Featured / বিশেষ' },
      { name: 'active', type: 'checkbox', label: 'Active / সক্রিয়', default: true },
    ],
  ...
  memories: {
    ...
    toggles: ['published', 'featured'],
    columns: [
      { labelKey: 'common.image', render: (i) => thumb(i.photos && i.photos[0] && i.photos[0].url) },
      { labelKey: 'common.title', render: (i, L) => <strong>{loc(i.title, L)}</strong> },
    ...
    mapForEdit: (item) => ({
      ...
      photoUrls: (item.photos || []).slice().sort((a, b) => a.displayOrder - b.displayOrder).map((p) => p.url),
    }),
    toPayload: (form) => ({
      title: form.title,
      description: form.description,
      ...
      photos: (form.photoUrls || []).map((url, i) => ({ url, caption: {}, displayOrder: i })),
  ...
  souvenirs: {
    ...
    fields: [
      { name: 'editionYear', type: 'number', label: 'Edition year / সংস্করণ বর্ষ', required: true },
      { name: 'title', type: 'localized', label: 'Title / শিরোনাম', required: true },
      { name: 'description', type: 'localizedText', label: 'Description / বিবরণ' },
      { name: 'coverImage', type: 'image', label: 'Cover image / কভার ছবি' },
      { name: 'pdfUrl', type: 'pdf', nameField: 'pdfName', label: 'PDF file / পিডিএফ' },
      { name: 'displayOrder', type: 'number', label: 'Order / ক্রম' },
      { name: 'featured', type: 'checkbox', label: 'Featured / বিশেষ' },
      { name: 'published', type: 'checkbox', label: 'Published / প্রকাশিত', default: true },
    ],
