...
/* ---------- Image with graceful fallback (no broken-image icons) ---------- */
export function Img({ src, alt = '', className = '', style, fallbackLabel, ...rest }) {
    ...
    return (
      <div
        ...
        style={{
          ...
          background: 'var(--cream-3)', color: 'var(--muted)', fontSize: '0.8rem',
          minHeight: 80, width: '100%', height: '100%', ...style,
        }}
        role="img"
  ...
  return <img src={src} alt={alt} className={className} style={style} loading="lazy" onError={() => setFailed(true)} {...rest} />;
}

/* ---------- Original club motif (alpana-inspired) ---------- */
export function MotifMark({ width = 32, height = 32, opacity = 1 }) {
  return (
    <svg width={width} height={height} viewBox="0 0 64 64" opacity={opacity} aria-hidden="true">
      <g fill="none" stroke="#ffffff" strokeWidth="2.4" strokeLinecap="round">
        <circle cx="32" cy="32" r="7" fill="#ffffff" stroke="none" opacity="0.95" />
        {Array.from({ length: 8 }).map((_, i) => {
          ...
          return (
            <g key={i}>
              <path d={`M${x1} ${y1} Q${cx} ${cy} ${x2} ${y2} Q${cx2} ${cy2} ${x1} ${y1}`} fill="#fff" fillOpacity="0.85" stroke="none" />
            </g>
          );
        })}
...
export function SectionHead({ eyebrow, title, text, center, tone }) {
  return (
    <div className={`section-head ${center ? 'center' : ''}`}>
      {eyebrow && <div className="section-eyebrow" style={tone === 'light' ? { color: 'var(--gold-soft)' } : undefined}>{eyebrow}</div>}
      {center ? (
        <>
          <div className="alpana-divider" aria-hidden="true"><MotifMark width={18} height={18} opacity={0.9} color="#b08a3e" /></div>
          <h2>{title}</h2>
        </>
      ) : (
        <h2>{title}</h2>
...
export function Pagination({ page, pages, onChange }) {
  ...
  return (
    <nav className="pager" aria-label="Pagination">
      ...
      <button type="button" className="btn btn-light btn-sm" disabled={page >= pages} onClick={() => onChange(page + 1)}>
        {t('common.next')}
      </button>
    </nav>
  );
...
export function Modal({ open, onClose, title, children, wide }) {
  ...
  useEffect(() => {
    ...
    // Runs only on open/close transitions — never on parent re-renders,
    // otherwise the delayed focus() steals focus from form inputs.
  }, [open]);
  if (!open) return null;
  return (
    <div
...
/* ---------- Confirm dialog ---------- */
export function ConfirmDialog({ open, onClose, onConfirm, title, text, danger = true, busy }) {
  const { t } = useLanguage();
  return (
    <Modal open={open} onClose={onClose} title={title || t('common.confirm')}>
      {text && <p style={{ color: 'var(--muted)' }}>{text}</p>}
      <div className="modal-actions">
        <button type="button" className="btn btn-light" onClick={onClose} disabled={busy}>{t('common.cancel')}</button>
...
export function Lightbox({ photos, index, onClose, onNavigate, captionFor }) {
  ...
  return (
    <div className="lightbox" role="dialog" aria-modal="true" aria-label={caption || 'Photo'}>
      ...
      {photos.length > 1 && (
        <>
          <button type="button" className="lightbox-btn lightbox-prev" onClick={() => go(-1)} aria-label="Previous photo">‹</button>
          <button type="button" className="lightbox-btn lightbox-next" onClick={() => go(1)} aria-label="Next photo">›</button>
        </>
      )}
