...
- Pages: About · **Durga Puja** (year-wise, schedules for ষষ্ঠী→দশমী style entries) · Events (+details) · Notices (search, pinned, PDF attachments) · Gallery (albums + lightbox, lazy loading) · Videos (YouTube/Vimeo embed **or uploaded video files** — MP4/WebM/MOV/MKV, native player, click-to-play) · Committee · **Members directory** (privacy-respecting) · Social Activities · Mansa Mandir · Memories · Souvenir (PDF) · Contact (map link + working form) · custom 404
...
- Secure uploads: MIME + extension allowlist, image-dimension validation, PDF + video magic-byte sniffing, size limits, randomized safe filenames; video uploads (default max 200 MB, `UPLOAD_MAX_VIDEO_MB`) stored under Media kind `video` with in-use deletion protection
...
| `UPLOAD_MAX_IMAGE_MB` / `UPLOAD_MAX_PDF_MB` / `UPLOAD_MAX_VIDEO_MB`      | Upload limits (defaults 5 / 15 / 200)                                                                                              |
...
_গোস্বামীপাড়া মনসামাতা ক্লাব · Established 2010 · Reg. S0020444 of 2021–2022 · Vill+Post Ramnagar, PS Tarakeswar, Hooghly 712410_.
...
GET/POST/PATCH/DELETE /api/media (…/upload, …/upload-pdf, …/upload-video, …/:id/replace)
GET             /api/admin/stats
